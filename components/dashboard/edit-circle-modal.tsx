    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
          <div className="bg-white border-2 border-neutral-200/60 rounded-2xl p-6 max-w-md w-full space-y-4 shadow-xl relative animate-in zoom-in-95 duration-250">
            {success ? (
               <div className="text-center space-y-4 py-4">
                 ...
                 <h3 className="font-bold text-neutral-900">Updated Successfully!</h3>
                 <p className="text-xs text-neutral-500 leading-relaxed">Your circle details have been updated matching current rules policies safely.</p>
                 <Button 
                   onClick={() => { 
                     setIsOpen(false); 
                     setSuccess(false); 
            ...
            <div>
              ...
              <p className="text-xs text-neutral-500 mt-1">Modify your savings pool configuration layout easily.</p>
            </div>

            {error && <p className="text-red-500 text-xs bg-red-50 p-2 rounded">{error}</p>}

            {!isAlone && (
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200/60 flex gap-2 items-start text-amber-700">
                <InfoIcon className="w-4 h-4 shrink-0 mt-0.5" />
                <p className="text-[10px] leading-relaxed">
                  Since you have other members, updating the <strong className="font-semibold">Name</strong> or <strong className="font-semibold">Contribution Amount</strong> will create a pending change request that must be approved by all members before making effect.
                </p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="space-y-1">
                <Label className="text-xs text-neutral-700">Circle Name</Label>
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  ...
                  className="w-full bg-white border border-neutral-200 rounded-md p-2 text-sm max-h-24"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <Label className="text-xs text-neutral-700">Max Members</Label>
              ...
              <div className="flex gap-2 justify-end pt-2 border-t border-neutral-100">
                <Button type="button" variant="outline" onClick={() => setIsOpen(false)} disabled={loading} className="text-xs">
                  Cancel
                </Button>
