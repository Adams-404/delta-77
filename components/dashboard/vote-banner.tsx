...
export function VoteBanner({ circle, viewerId }: { circle: any, viewerId: string }) {
  ...
  return (
    <div className="bg-amber-50 border border-amber-200/60 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in duration-200">
      <div className="flex gap-3 items-start md:items-center">
        ...
        <div>
          <h4 className="text-sm font-bold text-neutral-900">Proposed Update Pending</h4>
          <p className="text-xs text-neutral-600 mt-0.5 leading-relaxed">
            {circle.pendingName && <>Change Name to <strong className="font-semibold text-neutral-800">"{circle.pendingName}"</strong></>}
            {circle.pendingName && circle.pendingContributionAmount && <> and </>}
            {circle.pendingContributionAmount && <>Change Amount to <strong className="font-semibold text-neutral-800">₦{parseFloat(circle.pendingContributionAmount).toLocaleString()}</strong></>}
          </p>
          {isOrganizer && (
      ...
      {!isOrganizer && !hasVoted && (
        <div className="flex gap-2 shrink-0">
          ...
          <Button 
            ...
            disabled={loading}
            variant="outline"
            className="border-neutral-200 bg-white hover:bg-red-50 hover:text-red-600 hover:border-red-200 text-xs px-4 py-1.5 h-auto flex items-center gap-1 rounded-full"
          >
      ...
      {!isOrganizer && hasVoted && (
        <p className="text-xs font-semibold text-teal-600 flex items-center gap-1 shrink-0 bg-white px-3 py-1 rounded-full border border-teal-100 shadow-sm">
